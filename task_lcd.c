...
void lcd_init(void) {
	...
	Crystalfontz128x128_SetDrawFrame(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
 ...
 *  Nothing
 *******************************************************************************/
void lcd_draw_image(IMAGE toDraw) {

	uint16_t x_start = toDraw.x;
	uint16_t y_start = toDraw.y;
	uint16_t image_width_pixels = toDraw.width;
	uint16_t image_height_pixels = toDraw.height;
    uint8_t *image = toDraw.image;
	uint16_t fColor = toDraw.fColor;
	uint16_t bColor = toDraw.bColor;
 ...
 *  Nothing
 *******************************************************************************/
void lcd_draw_line_horizontal(uint16_t x, uint16_t y, uint16_t len,
		uint16_t fColor) {
	// if larger than screen size, set a limit
	int i;
	uint16_t x1 = (len + x >= SCREEN_WIDTH) ? SCREEN_WIDTH : x + len;
 ...
 *  Nothing
 *******************************************************************************/
void lcd_draw_line_vertical(uint16_t x, uint16_t y, uint16_t len,
		uint16_t fColor) {
	int i;
	uint16_t y1 = (len + y >= SCREEN_HEIGHT) ? SCREEN_HEIGHT : y + len;
 ...
 *  Nothing
 *******************************************************************************/
void lcd_draw_pixel(uint16_t x, uint16_t y, uint16_t fColor) {
...
void Task_LCD(void *pvParameters) {
	IMAGE toDraw; // parameters for drawing LCD

	while (1) {
		// receive parameters for drawing LCD from queue
		xQueueReceive(Queue_LCD, &toDraw, portMAX_DELAY);

		// draw LCD with received parameters
		lcd_draw_image(toDraw);
	}
}
